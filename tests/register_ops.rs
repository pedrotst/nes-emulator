...
fn test_0xaa_tax_move_a_to_x() {
    let mut cpu = CPU::mock_cpu(vec![0xaa, 0x00]);
    cpu.register_a = 10;
...
fn test_0xaa_txa_move_a_to_x() {
    let mut cpu = CPU::mock_cpu(vec![0x8a, 0x00]);
...
#[test]
fn test_tay(){
    let mut cpu = CPU::mock_cpu(vec![0xa8, 0x00]);
    // Let's test a random negative number to test flag setting
...
#[test]
fn test_tya(){
    let mut cpu = CPU::mock_cpu(vec![0x98, 0x00]);
    // Let's test a random negative number to test flag setting
...
fn inx_overflow_zero_flag() {
    let mut cpu = CPU::mock_cpu(vec![0xe8, 0x00]);
    cpu.register_x = 0xff;
    //cpu.load_and_run_no_reset();
    cpu.run();
...
fn dex_underflow_negative_flag() {
    let mut cpu = CPU::mock_cpu(vec![0xca, 0x00]);
    cpu.register_x = 0x00;
    //cpu.load_and_run_no_reset();
    cpu.run();
...
fn iny_overflow_zero_flag() {
    let mut cpu = CPU::mock_cpu(vec![0xc8, 0x00]);
    cpu.register_y = 0xff;
    //cpu.load_and_run_no_reset();
    cpu.run();
...
fn dey_underflow_negative_flag() {
    let mut cpu = CPU::mock_cpu(vec![0x88, 0x00]);
    cpu.register_y = 0x00;
    //cpu.load_and_run_no_reset();
    cpu.run();
